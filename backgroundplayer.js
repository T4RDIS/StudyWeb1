/**
 * Created by kdudkowski on 19.12.15.
 */
...
var background = (function($){
    ...
    function backgroundplayer(setting){
        ...
        var $playcontrol = (function(){ // define plus reference play button within audioui
            ...
            var controlvals = (controltype=='src')? [$control.data('playsrc'), $control.data('pausesrc')] : ['Odtwórz', 'Pauza'] // get label for each button state (play and pause)
            if (autoplay){
            ...
            $control
                .attr('title', (!autoplay)? 'Odtwórz' : 'Pauza')
        ...
        var $volcontrol = $audioui.find('div.volume:eq(0)') // define volume control
            ...
            .html((function(){
                for (var sliderhtml='', i=1; i<=s.volumelevels; i++) // create the individual volume bars
                    sliderhtml += '<div data-vol="' +  i/s.volumelevels +'" title="Ustaw głośność: ' + Math.round(i/s.volumelevels*100) +'%"></div>'
                return sliderhtml
            }()))
